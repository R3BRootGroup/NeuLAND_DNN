...
if __name__ == "__main__":
    ...
    TheNetworkName = ThisOutputPath + 'TheNEBULAMultNetwork_Epoch' + TheEpochStr + '.h5' # ATTENTION: NEBULA!
        ...
        for kBatch in range(0,SetParameters.nBatches):
            ...
            if ((InputData.shape[0]<=SetParameters.BatchSize) and 
                ...
                for n in range(0,nEventsPerBatch):
                    ...
                    EventMultiplicity = MaxIndex_Predicted # ATTENTION: NEBULA!
                        ...
                        if (EventMultiplicity==SetParameters.Acc_Selected_Multiplicity):
                            ...
                            if (MaxIndex_Predicted==0):
                                ...
                                if (MaxProb>(UpperMaxProb + SetParameters.Acc_ProbThreshold)):
                                    EventMultiplicity = 0 # ATTENTION: NEBULA!
                                else:
                                    EventMultiplicity = 1 # ATTENTION: NEBULA!
                                ...
                                if ((MaxProb>(LowerMaxprob + SetParameters.Acc_ProbThreshold)) and (MaxProb>(UpperMaxProb + SetParameters.Acc_ProbThreshold))):
                                    EventMultiplicity = MaxIndex_Predicted+0 # ATTENTION: NEBULA!
                      
                                else:
                                    
                                    # Then, check wich probability is larger:
                                    if (LowerMaxprob>UpperMaxProb):
                                        EventMultiplicity = MaxIndex_Predicted-1 # ATTENTION: NEBULA!
                    ...
                    # And redefine the max. index:
                    MaxIndex_Predicted = EventMultiplicity # ATTENTION: NEBULA!
                ...
                print('Executed NEBULA Netwok prediction from batch '+str(kBatch+1)+'/'+str(SetParameters.nBatches)+'.') # ATTENTION: NEBULA!
        
            else:
          
                # Give error message:
                print('==> ERROR in NEBULA batch '+str(kBatch+1)+'/'+str(SetParameters.nBatches)+': MATRIX DIMENSIONS WERE INCORRECT!') # ATTENTION: NEBULA!
