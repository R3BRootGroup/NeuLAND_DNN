...
if __name__ == "__main__":
    ...
    if (TheLabel>0):
        NetworkName = NetworkName + 'Step2Network_NEBULA_Mult' + TheLabelStr + '_Epoch0.h5' # ATTENTION: NEBULA!
    else:
        NetworkName = NetworkName + 'TheNEBULAMultNetwork_Epoch0.h5' # ATTENTION: NEBULA!
    ...
    for kEpoch in range(0,SetParameters.nEpochs):
        ...
        for kBatch in range(0,SetParameters.nBatches):
            ...
            InputFileName = SetParameters.TextFilePath + 'DNN_NEBULA_InputFile_' + str(InitialEventCounter) + 'till' + str(FinalEventCounter) + '.txt' # ATTENTION: NEBULA!
            OutputFileName = SetParameters.TextFilePath + 'DNN_NEBULA_InputFile_Outputs_' + str(InitialEventCounter) + 'till' + str(FinalEventCounter) + '.txt' # ATTENTION: NEBULA!
            ...
            if ((InputTest==True) and (OutputTest==True)):
                ...
                if (BatchDimensions==True):
                    ...
                    print('Executed NEBULA Netwok update from batch '+str(kBatch+1)+' from '+str(SetParameters.nBatches)+' in epoch '+str(kEpoch+1)+'/'+str(SetParameters.nEpochs)+'.') # ATTENTION: NEBULA!
                
                else:
                    # Give a warning:
                    print('NEBULA Netwok update from batch '+str(kBatch+1)+' from '+str(SetParameters.nBatches)+' in epoch '+str(kEpoch+1)+'/'+str(SetParameters.nEpochs)+' was skipped because the matrix dimensions were incorrect!') # ATTENTION: NEBULA!
                ...
                print('NEBULA Netwok update from batch '+str(kBatch+1)+' from '+str(SetParameters.nBatches)+' in epoch '+str(kEpoch+1)+'/'+str(SetParameters.nEpochs)+' was skipped because Input/Output files are not present!') # ATTENTION: NEBULA!
        ...
        if (SetParameters.SaveBetweenEpochs==True):
            ...
            if (TheLabel>0):
                CurrentNetworkName = CurrentNetworkName + 'Step2Network_NEBULA_Mult' + TheLabelStr + '_Epoch' + TheEpochStr + '.h5' # ATTENTION: NEBULA!
            else:
                CurrentNetworkName = CurrentNetworkName + 'TheNEBULAMultNetwork_Epoch' + TheEpochStr + '.h5' # ATTENTION: NEBULA!
        ...
        print('###: NEBULA EPOCH '+str(kEpoch+1)+' FROM '+str(SetParameters.nEpochs)+' WAS COMPLETED!') # ATTENTION: NEBULA!
        print('')
    ...
    if (TheLabel>0):
        FinalNetworkName = FinalNetworkName + 'Step2Network_NEBULA_Mult' + TheLabelStr + '_Final.h5' # ATTENTION: NEBULA!
    else:
        FinalNetworkName = FinalNetworkName + 'TheNEBULAMultNetwork_Final.h5'
