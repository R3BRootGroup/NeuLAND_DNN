...
void R3BMultMatrix::ReadNEBULACalibrationFile()
{
    ...
    { 
        ...
        if (MaxMultiplicity==(NLines-1))
        {
            ...
            // give some output:
            cout << "-I- R3BMultMatrix: Max. NEBULA Multiplicity = " << MaxMultiplicity << "\n"
        ...
        {
            cout << "### ERROR: R3BMultMatrix: The file holding the NEBULA calibration-cuts does not correspond to the current max. multiplicity in the Inputs-file!\n\n";
        }
    }
}

// Function to apply the calibration cuts inside the Exec-function:
Int_t R3BMultMatrix::ApplyNEBULACalibrationCuts()
{
    // Declare the answer:
    ...
    if (UseNEBULACalibrationCuts==kTRUE)
    {
        ...
        Int_t nClusters = fArrayNEBULACluster->GetEntries();
        ...
        for (Int_t k = 0; k<nClusters; k++) 
        {
            ...
            ThisCluster = (R3BSignalCluster*) fArrayNEBULACluster->At(k);
        ...
        else
        {
            ...
            {
                std::cout << "### R3BMultMatrix: SERIOUS ERROR IN APPLYING THE NEBULA CUTS!!!\n\n";
            }
        }
