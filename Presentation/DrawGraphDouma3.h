...
TCanvas* DrawGraphDouma3(TGraphErrors* const Raw, TString const Title, TString const xlabel, TString const ylabel, Int_t const Color, Double_t const ScaleFactor)
{
  ...
  (Raw->GetXaxis())->SetLimits(Xmin-0.0*Xlength,Xmax+0.0*Xlength);
  
  Double_t Ymin = 0.15;
  Double_t Ymax = 0.65;
  Double_t Ylength = Ymax - Ymin;
  
  Raw->SetMinimum(Ymin-0.1*Ylength);
  Raw->SetMaximum(Ymax+0.1*Ylength);
  
  (Raw->GetYaxis())->SetLimits(Ymin-0.1*Ylength,Ymax+0.2*Ylength);
