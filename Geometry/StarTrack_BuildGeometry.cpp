...
void StarTrack_BuildGeometry()
{
  ...
  Double_t X_position = Inputs->GetInputDouble("STARTRACK_center_x_position","cm");
  Double_t Y_position = Inputs->GetInputDouble("STARTRACK_center_y_position","cm");
  Double_t Z_position = Inputs->GetInputDouble("STARTRACK_center_z_position","cm");
  
  Double_t Rotation_X = Inputs->GetInputDouble("STARTRACK_x_rotation_angle","deg");
  Double_t Rotation_Y = Inputs->GetInputDouble("STARTRACK_y_rotation_angle","deg");
  Double_t Rotation_Z = Inputs->GetInputDouble("STARTRACK_z_rotation_angle","deg");
  
  // Define the file name:
  TString geoFileName = Inputs->GetInputString("TheOutputPath") + Inputs->GetInputString("STARTRACK_Geometry_FileName");
  ...
  // StarTrack world (mother volume to have vacuum for Si tracker and air for CALIFA)  !!
   ...
   t0->SetTranslation(X_position,Y_position,Z_position+ShiftToWorldEdge);
   
   // add the sphere as Mother Volume
   pAWorld->AddNode(pTraWorld, 0, t0);
 ...
 //dz = -(Length3+Frame_Length)*cos(InclAng3*PI/180.)/2 + (Rmin3/tan(AngRangeMin3*PI/180.));
  ...
  STaRTraCBFrameLog1->RegisterYourself();
  ...
  STaRTraCBFrameLog3->RegisterYourself();
  ...
  STaRTraCBFrameLog2->RegisterYourself();
